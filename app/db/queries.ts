...
export async function getFoldersWithEmailCount(): Promise<{ specialFolders: Folder[]; otherFolders: Folder[] }> {
  try {
    ...
    console.error('Error in getFoldersWithEmailCount:', error);
    throw error;
  }
}

export async function getEmailsForFolder(folderName: string, search?: string): Promise<EmailWithSender[]> {
  try {
    ...
    if (search === undefined) {
      ...
      `;
    } else if (originalFolderName === 'Sent') {
        result = await sql`
          SELECT e.*, u.first_name, u.last_name, u.email
          FROM emails e
          JOIN email_folders ef ON e.id = ef.email_id
    ...
    console.error('Error in getEmailsForFolder:', error);
    throw error;
  }
}

export async function getEmailInFolder(folderName: string, emailId: string): Promise<EmailWithSender> {
  try {
    const originalFolderName = toTitleCase(decodeURIComponent(folderName));
    const result = await sql`
      SELECT e.*, u.first_name, u.last_name, u.email
    ...
    console.error('Error in getEmailInFolder:', error);
    throw error;
  }
}
...
export async function getAllEmailAddresses(): Promise<UserEmail[]> {
  try {
    ...
    console.error('Error in getAllEmailAddresses:', error);
    throw error;
  }
}
